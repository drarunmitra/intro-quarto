...
df <- read_rds("kerala_covid_deaths.rds")
```
...
```{r}
# Hint: select() , see the help file at 
# sl_no, name, place, history_traveler_contact

df <- df %>% dplyr::select(!c(sl_no, name, place, history_traveler_contact))
```
...
```{r}
df %>% count(date_reported) %>% arrange(-n)
```
...
```{r}
# Hint: arrange?
df %>% count(district) %>% arrange(-n) %>% slice(1:5)
```
...
df %>% mutate(delay = date_of_death - date_reported) %>% pull(delay) %>% mean(na.rm = T)
```
...
```{r}
# Hint: group_by()
df %>% mutate(delay = date_of_death - date_reported) %>% group_by(district) %>% summarize(mean_delay = mean(delay, na.rm = T))
```
...
df <- df %>% mutate(age_group = ifelse(age <= 60, "<60 Years",">60 Years"))
df <- df %>% mutate(age_group = case_when(age <= 60 ~ "<60 Years", TRUE ~ ">60 Years"))
...
```{r}
# Hint: ifelse(), case_when()
df <- df %>% mutate(wave = ifelse(date_of_death <= "2021-04-01",
                            "First Wave",
                            "Second Wave"))  


df <- df %>% mutate(wave = case_when(date_of_death <= "2021-04-01" ~ "First Wave",
                                     TRUE ~ "Second Wave"))
```
